...
public class PurchaseController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        for (ProductDTO item : productMap.keySet()) {
             if(new ProductDAO().getCurrentStock(item.getId())-productMap.get(item) <0){
                ...
                total += item.getOriginalSalePrice()*productMap.get(item)*1.1;
             }
       ...
       // If a item list is valid => Update database
        if(isValid){
            ...
            orderDao.createOrder(new OrderDTO(0, currentUser.getEmail(), "submitted", null, createDate, total), itemList);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        orderDao.createOrder(new OrderDTO(0, "user@fpt.edu.vn", "pending", "sale@fpt.edu.vn", new Date(Calendar.getInstance().getTimeInMillis()),total), itemList);
        ss.setAttribute("productList", null);
        response.sendRedirect("Cart.jsp");
    }
